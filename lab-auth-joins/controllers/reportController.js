...
exports.latestLogin = (req, res) => {
  const sql = `
    ...
    FROM users u
    LEFT JOIN (
      SELECT la1.user_id, la1.ip_address, la1.occurred_at
      FROM login_audit la1
      INNER JOIN (
        SELECT user_id, MAX(occurred_at) AS max_date
        FROM login_audit
        GROUP BY user_id
      ) la2 ON la1.user_id = la2.user_id AND la1.occurred_at = la2.max_date
